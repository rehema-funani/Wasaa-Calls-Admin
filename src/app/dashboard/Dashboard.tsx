...
const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        activeCalls: callsData.activeCalls || 342,
        escrowLocked: billingData.totalLocked || 120500,
        avgCallDuration: callsData.avgDuration || 12,
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={qosData} margin={{ top: 5, right: 20, left: -15, bottom: 5 }}>
                ...
                <YAxis stroke="var(--color-text-secondary)" fontSize={12} tickLine={false} axisLine={false} domain={[80, 100]} tickFormatter={(value) => `${value}%`} />
                <Tooltip 
                  contentStyle={{ 
                ...
                />
                <Area type="monotone" dataKey="score" stroke="var(--color-primary-500)" strokeWidth={2} fillOpacity={1} fill="url(#colorQoS)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
            ...
            {recentActivity.map((activity) => {
              ...
              return (
                <div key={activity.id} className="flex items-start space-x-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className={`w-2 h-2 ${colorClass.bg} rounded-full mt-2 ${activity.type === 'call_start' ? 'animate-pulse' : ''}`}></div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-900 dark:text-white">{renderActivityMessage(activity)}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{formatTimeAgo(activity.time)}</p>
                  </div>
                  <ActivityIcon size={16} className={`${colorClass.text} mt-0.5`} />
                </div>
              );
            })}
          </div>
